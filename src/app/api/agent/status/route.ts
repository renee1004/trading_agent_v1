    ...
    return NextResponse.json({
      ...
      data: {
        ...
        lastCycleSummary: agentStatus.lastCycleResult ? {
          stocksAnalyzed: agentStatus.lastCycleResult.stocksAnalyzed ?? 0,
          signalsGenerated: agentStatus.lastCycleResult.signalsGenerated ?? 0,
          ordersPlaced: agentStatus.lastCycleResult.ordersPlaced ?? 0,
          positionsMonitored: agentStatus.lastCycleResult.positionsMonitored ?? 0,
          exitsExecuted: agentStatus.lastCycleResult.exitsExecuted ?? 0,
          duration: agentStatus.lastCycleResult.endTime.getTime() - agentStatus.lastCycleResult.startTime.getTime(),
          domesticSuccess: agentStatus.lastCycleResult.domesticSuccess ?? 0,
          domesticFailed: agentStatus.lastCycleResult.domesticFailed ?? 0,
          overseasSuccess: agentStatus.lastCycleResult.overseasSuccess ?? 0,
          overseasFailed: agentStatus.lastCycleResult.overseasFailed ?? 0,
          zeroAnalysisReason: agentStatus.lastCycleResult.zeroAnalysisReason ?? null,
          // ── KIS 호출 단계별 카운트 (이전 객체 누락 방지: ?? 0) ──
          candleSuccess: agentStatus.lastCycleResult.candleSuccess ?? 0,
          candleFailed: agentStatus.lastCycleResult.candleFailed ?? 0,
          priceSuccess: agentStatus.lastCycleResult.priceSuccess ?? 0,
          priceFailed: agentStatus.lastCycleResult.priceFailed ?? 0,
          balanceSuccess: agentStatus.lastCycleResult.balanceSuccess ?? 0,
          balanceFailed: agentStatus.lastCycleResult.balanceFailed ?? 0,
          // ── 진단 필드 ──
        ...
        kisDiagnostics: {
          ...
          ...(getKisLastError() ? {
            lastError: {
              phase: getKisLastError()!.phase,
              stockCode: getKisLastError()!.stockCode ?? null,
              endpointPath: getKisLastError()!.endpointPath ?? null,
              trId: getKisLastError()!.trId ?? null,
              server: getKisLastError()!.server,
              httpStatus: getKisLastError()!.httpStatus,
              rt_cd: getKisLastError()!.rt_cd ?? null,
              msg_cd: getKisLastError()!.msg_cd ?? null,
              msg1: getKisLastError()!.msg1 ?? null,
              probableCauses: getKisLastError()!.probableCauses,
              timestamp: getKisLastError()!.timestamp,
            },
          } : {}),
        },
      },
